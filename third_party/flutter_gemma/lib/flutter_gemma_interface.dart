...
abstract class InferenceModelSession {
  ...
  // PATCH (harcapp): clears any buffered query chunks so the session can be
  // reused for an independent, stateless prompt without accumulating previous
  // turns. Default no-op; overridden where it matters (MediaPipe web). Lets a
  // caller reuse ONE session for many one-shot prompts — needed because on
  // MediaPipe web, closing a session and creating another reuses a dead cached
  // LlmInference (createSession is idempotent per model).
  Future<void> clearQueryChunks() async {}
