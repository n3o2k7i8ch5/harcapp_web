...
class WebModelSourceResolver {
  ...
  resolveActiveInferenceModel() async {
    // PATCH (harcapp): forActiveModel() builds a fresh WebModelManager whose
    // active model is rehydrated from prefs only by ensureInitialized(). The
    // upstream code read `activeInferenceModel` synchronously here without
    // awaiting init, so it was always null on a fresh manager (→ "No active
    // inference model set"). Await init first so rehydration runs.
