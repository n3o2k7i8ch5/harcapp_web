...
class WebModelManager extends ModelFileManager {
  // PATCH (harcapp): make WebModelManager a true singleton. Upstream both
  // FlutterGemmaWeb (the "singleton" it documents) AND
  // WebModelSourceResolver.forActiveModel() did `WebModelManager()`, getting
  // SEPARATE instances. The active model set by install() lived only in the
  // FlutterGemmaWeb instance's memory, so the resolver's fresh instance saw no
  // active model on the first createSession (→ "No active inference model set";
  // its rehydrate-from-prefs also raced the unawaited persist). One shared
  // instance fixes both: the resolver sees the in-memory active model directly.
  static final WebModelManager _instance = WebModelManager._();
  factory WebModelManager() => _instance;
  WebModelManager._();
