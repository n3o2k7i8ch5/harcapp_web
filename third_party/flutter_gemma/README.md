<!--
  ┌───────────────────────────────────────────────────────────────────────────┐
  │  harcapp_web NOTE — this is a VENDORED, PATCHED copy of flutter_gemma 1.0.1 │
  └───────────────────────────────────────────────────────────────────────────┘

  This is NOT a normal pub.dev dependency. It is a local copy of flutter_gemma
  1.0.1, wired in via `dependency_overrides:` in the project pubspec.yaml, with a
  small bug fix applied.

  WHY IT EXISTS:
  flutter_gemma 1.0.1 has a web-only bug that makes on-device inference fail on
  the FIRST createSession() after installing a model:

      Bad state: No active inference model set. Use FlutterGemma.installModel() first.

  Root cause: both FlutterGemmaWeb and WebModelSourceResolver.forActiveModel()
  did `WebModelManager()` and got SEPARATE instances. install() set the active
  model only on FlutterGemmaWeb's instance, so the resolver's fresh instance saw
  no active model (and its rehydrate-from-prefs raced an unawaited persist).

  We can't edit a package in ~/.pub-cache (read-only, overwritten on pub get),
  upstream 1.0.1 has no fix, so we vendor a copy and override it here. The fix is
  one place: WebModelManager is now a true singleton (factory constructor). See
  PATCH.md for full details — search the source for `PATCH (harcapp)`.

  TO REMOVE once upstream fixes it: delete this folder, drop the
  `dependency_overrides: flutter_gemma:` block in pubspec.yaml, bump the version.
